# -*- coding: utf-8 -*-
"""
[65] lr 확증 — exp/58에서 lr0.08→0.04가 +53.9 (d6/it500 동일조건). 2시드 확증
...
NTHREAD = 8
REF_SEED42 = 777.0
REF_2SEED = 800.9   # exp/45 both 2시드
SEEDS = [42, 7]
K_MIX = 50.0
...
ARMS2 = [("d6_lr04", dict(iterations=500, depth=6, learning_rate=0.04)),
         ("d8_lr04", dict(iterations=250, depth=8, learning_rate=0.04)),
         ("d6_lr02", dict(iterations=1000, depth=6, learning_rate=0.02)),
         ("d8_lr02", dict(iterations=500, depth=8, learning_rate=0.02))]
...
DEN = r_ * (1 - r_)
best = {}
for name, prm in ARMS2:
    ps = []
    for sd in SEEDS:
        t0 = time.time()
        m = CatBoostClassifier(**{**BASE_CB, "random_seed": sd}, **prm).fit(ptr)
        p = m.predict_proba(pva)[:, 1]
        ps.append(p)
        tick(f"{name} seed={sd} {raw_score(p, y_va):8.1f}  ({time.time()-t0:.0f}s)")
        del m
    ens = np.mean(ps, axis=0)
    np.save(f"lab/65_{name}.npy", ens.astype("float32"))
    scv = raw_score(ens, y_va)
    pen = 100000 * (ens.mean() - r_) ** 2 / DEN
    best[name] = (scv, scv + pen, pen)
    log(f"  -> {name:9s} 2seed {scv:8.1f} (ref {REF_2SEED} 대비 {scv-REF_2SEED:+.1f}) "
        f"| 변별력 {scv+pen:.1f} 벌점 {pen:.1f}")

log("")
log("=" * 80)
log("판정 (2024 폴드, 2시드 앙상블 · 기준 ref 2시드 800.9)")
log("=" * 80)
log(f"  {'ref(lr0.08/d6/it500)':24s} {REF_2SEED:8.1f}")
for k, (scv, disc, pen) in best.items():
    log(f"  {k:24s} {scv:8.1f}  ({scv-REF_2SEED:+.1f})  변별력 {disc:.1f} / 벌점 {pen:.1f}")
log("")
log("  · 승자 설정으로 멀티클래스 재학습 -> 새 챔피언 후보")
log("  · 변별력 기여가 커야 진짜 (중심벌점만 줄면 전이 위험)")
log("=" * 80)
