# -*- coding: utf-8 -*-
"""
[99] 미세 피처(P1/P2/P3) 짝지음 ablation — 챔피언 cat5 위, exp/93 하네스 그대로 (수치형 피처 append, cat 불변)

팔 (thread14 · 시드[42,7] · it500/d6/lr0.08/l2=10 · MultiClass 5클래스 · cat5):
  MID    + f_mid_idx    = z(asof_pitcher_middle_rate) + z(asof_pitcher_prev5_game_middle_rate) + z(asof_batter_middle_rate)
                          z 의 mean/std 는 학습행(≤2023)에서 얼린 스칼라 6개. 결측 성분은 0(=평균) 처리. (P2)
  BE     + f_b_early    = 1[31 ≤ asof_batter_n ≤ 100]                                            (P3)
  MIDBE  MID + BE                                                                                (제미나이 제안 묶음)
  RK     + f_rk_ratedev = (asof_pitcher_success_rate − prior) × 1[시즌 전 커리어 n == 0]           (P1, HANDOFF §1.16)
                          rookie 판정 = CS 상수표의 N_end == 0 (pitcher_id 가 ≤학습연도 표에 없음). 2019 는 표가 없어
                          전원 N_end==0 이 되므로 NaN 처리 (데이터가 2019 부터라 2019 의 asof_n==0 은 신인이 아님).
                          rate 결측 신인은 편차 0.
  ALL    MID + BE + RK
전부 '현재 행 값 + 학습 유래 스칼라/ID표' 만 사용 → 행 독립. 냉동 타겟 통계표 없음.

기준: lab/89_cat5_probs_seed{42,7}.npy (851.2 / 856.5). 출력: HANDOFF §1.9 표준 6줄 + d/K,
      lab/99_{arm}_probs_seed{sd}.npy, lab/99_{arm}.npy, lab/99_summary.txt 한 줄 append
실행: PYTHONIOENCODING=utf-8 python -u exp/99_microfeat.py --arm MIDBE [--smoke]
"""
...
ap.add_argument("--arm", default="MIDBE", choices=["MID", "BE", "MIDBE", "RK", "ALL"])
...
F_MID, F_BE, F_RK = "f_mid_idx", "f_b_early", "f_rk_ratedev"
ARM_FEATS = {"MID": [F_MID], "BE": [F_BE], "MIDBE": [F_MID, F_BE], "RK": [F_RK],
             "ALL": [F_MID, F_BE, F_RK]}
MID_COLS = ["asof_pitcher_middle_rate", "asof_pitcher_prev5_game_middle_rate",
            "asof_batter_middle_rate"]

log(f"=== exp/99 arm={ARM} feats={ARM_FEATS[ARM]} smoke={args.smoke} ===")
log("train 로딩...")
df = load_train()

# ---- 라벨 복원 (exp/89·93 과 동일) ----
...
for S in sorted(tr_rows.season.unique()):
    ...
    part = s12.attach_cs(rows, cpS, cbS)
    # rookie = 이 시즌 전 커리어 없음 (상수표에 없음). 2019 는 표 자체가 없으므로 판정 불가 → NaN
    if len(h) == 0:
        part["_rookie"] = np.nan
    else:
        part["_rookie"] = (~part.pitcher_id.isin(cpS["id"])).astype("float32").to_numpy()
    parts.append(part)
...
va = s12.attach_pt(s12.add_features(s12.attach_cs(va_rows, cp, cb_), prior),
                   mix_dep, pfb)
va["_rookie"] = (~va.pitcher_id.isin(cp["id"])).astype("float32").to_numpy()
del va_rows, df
gc.collect()

# ---- 미세 피처 (행 독립: 현재 행 값 + 학습 유래 스칼라) ----
Z_CONST = {c: (float(np.nanmean(tr[c].to_numpy("float64"))),
               float(np.nanstd(tr[c].to_numpy("float64")))) for c in MID_COLS}
...
def add_micro(d):
    ...
    for c in MID_COLS:
        mu, sd = Z_CONST[c]
        zsum += np.nan_to_num((d[c].to_numpy("float64") - mu) / sd, nan=0.0)
    d[F_MID] = zsum.astype("float32")
    bn = d["asof_batter_n"].fillna(0).to_numpy("float64")
    d[F_BE] = ((bn >= 31) & (bn <= 100)).astype("int8")
    dev = np.nan_to_num(d["asof_pitcher_success_rate"].to_numpy("float64") - prior, nan=0.0)
    rk = d["_rookie"].to_numpy("float64")
    d[F_RK] = np.where(np.isnan(rk), np.nan, dev * rk).astype("float32")
    return d


tr = add_micro(tr)
va = add_micro(va)
log("z 상수(≤2023): " + "  ".join(f"{c}: mu={m:.4f} sd={s:.4f}" for c, (m, s) in Z_CONST.items()) + f"   prior={prior:.5f}")
log(f"학습행 신인 비율 {np.nanmean(tr['_rookie']):.3f} (2019 NaN 제외)  검증행 신인 비율 {va['_rookie'].mean():.3f} "
    f"(신인 투수 {va.loc[va._rookie==1,'pitcher_id'].nunique()}명)")
log(f"검증 f_b_early 비율 {va[F_BE].mean()*100:.2f}%  f_mid_idx 평균/표준편차 {va[F_MID].mean():+.3f}/{va[F_MID].std():.3f}  "
    f"f_rk_ratedev 비영 비율 {(va[F_RK]!=0).mean()*100:.2f}%")
tick("피처 부착 완료")
...
if args.smoke:
    ...
    log("smoke 새 피처 중요도: " + ", ".join(f"{k}={imp[k]:.2f}" for k in ARM_FEATS[ARM]))
    log("smoke 새 피처 예시(검증 앞 3행): " + " | ".join(f"{c}={Xva[c].iloc[:3].tolist()}" for c in ARM_FEATS[ARM]))
    log("smoke 새 피처 NaN 비율: " + ", ".join(f"{c}={Xva[c].isna().mean()*100:.2f}%" for c in ARM_FEATS[ARM]))
    sys.exit(0)
...
for sd in SEEDS:
    ...
    prob = m.predict_proba(pva)
    assert list(m.classes_)[0] == 0, f"클래스0 이 첫 열이 아님: {m.classes_}"
    np.save(f"lab/99_{ARM}_probs_seed{sd}.npy", prob.astype("float32"))
    p = prob[:, 0]
    preds.append(p)
    ...
    log("   새 피처 중요도: " + ", ".join(f"{k}={imp[k]:.2f}(#{int(rk[k])})" for k in ARM_FEATS[ARM]))
    log("   참고 팀ID 중요도: " + ", ".join(f"{k}={imp[k]:.2f}(#{int(rk[k])})" for k in ["pitcher_team_id", "batter_team_id"]))
    del m, prob
    gc.collect()

ens = np.mean(preds, axis=0)
np.save(f"lab/99_{ARM}.npy", ens.astype("float32"))
...
gt = None
rookie_va = np.load("lab/98_rookie_mask.npy") if ARM in ("RK", "ALL") else None
try:
...
log("")
log("=" * 80)
log(f"arm {ARM} ({'+'.join(ARM_FEATS[ARM])})  2시드 {e2:7.1f}  시드평균 {ms:7.1f}  proj8 {p8:7.1f}  벌점 {pen:5.1f}")
...
extra = ""
if gt is not None:
    extra += "   game_type별 2시드 이득: " + "  ".join(
        f"{g} {raw_score(ens[gt==g], y_va[gt==g]) - raw_score(p_base[gt==g], y_va[gt==g]):+.1f}" for g in sorted(set(gt)))
if rookie_va is not None and len(rookie_va) == len(ens):
    rm = rookie_va.astype(bool)
    extra += (f"   신인행({rm.mean()*100:.1f}%) 이득 {raw_score(ens[rm], y_va[rm]) - raw_score(p_base[rm], y_va[rm]):+.1f}"
              f" / 비신인 {raw_score(ens[~rm], y_va[~rm]) - raw_score(p_base[~rm], y_va[~rm]):+.1f}")
log(f"4. 연도별 gain                  2024 폴드 단일 ({dK:+.1f}) — 다연도 미측정" + extra)
log(f"5. 냉동 표 의존                 없음 — z 상수 6개·prior 스칼라·rookie 는 CS 상수표 N_end==0 (기존 CS 축과 같은 ID 표). [S] 구조형")
...
log("=" * 80)
with open("lab/99_summary.txt", "a", encoding="utf-8") as f:
    print(f"arm={ARM} feats={ARM_FEATS[ARM]} nfeat={len(FEATS)} solo={[round(float(x),2) for x in solo]} ens={e2:.1f} seedmean={ms:.1f} "
          f"proj8={p8:.1f} pen={pen:.1f} paired={[round(float(x),2) for x in pairs]} shape={sh_new-sh_base:+.1f} d={dK:+.1f} K={K:.1f}", file=f)
