# -*- coding: utf-8 -*-
"""
[64] Trackman 증류 확증 (clean) — teacher(현재 투구 물리값) → student(투구 전 정보만)
...
# ★ df의 dtype을 바꾸지 않는다 — 조인용 키는 사본에서만 캐스팅
KEYNUM = ["season", "game_month", "game_dayofweek", "inning",
          "balls_before", "strikes_before", "outs_before"]
_kf = df[KEY].copy()
for c in KEYNUM:
    _kf[c] = pd.to_numeric(_kf[c], errors="coerce").astype("Int64")
    tm[c] = pd.to_numeric(tm[c], errors="coerce").astype("Int64")
_kf["_ridx"] = np.arange(len(df))
trk = _kf.dropna(subset=["pt", "bt", "ph", "bh"])
...
_j = _kf.merge(tm11, on=KEY, how="left", validate="m:1").sort_values("_ridx")
for c in PHYS + ["pitch_type_group"]:
    df[c] = _j[c].to_numpy()
...
for sd in SEEDS:
    t0 = time.time()
    b_ = CatBoostClassifier(**CB, random_seed=sd).fit(
        Pool(Xtr, y_tr, cat_features=list(s12.CAT)))
    bl.append(b_.predict_proba(Pool(Xva, cat_features=list(s12.CAT)))[:, 1])
    tick(f"기준 student seed={sd}: {raw_score(bl[-1], y_va):.1f} ({time.time()-t0:.0f}s)")
    del b_
p_base1 = np.mean(bl, axis=0)
s_base1 = raw_score(p_base1, y_va)
np.save("lab/64_base.npy", p_base1.astype("float32"))
log(f"  기준 student {len(SEEDS)}시드 앙상블: {s_base1:.1f}")
...
for a in ALPHAS:
    ...
    for sd in SEEDS:
        ...
        ps.append(st.predict_proba(Pool(Xva, cat_features=list(s12.CAT)))[:, 1])
        del st
    p = np.mean(ps, axis=0)
    sc = raw_score(p, y_va)
    results[a] = sc
    np.save(f"lab/64_alpha{int(a*100)}.npy", p.astype("float32"))
