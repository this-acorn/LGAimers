...
def main():
    ...
    required = [source / 'script.py', source / 'model/model.pkl', data_dir / 'test.csv']
    for path in required:
        if not path.is_file():
            parser.error(f'Required file is missing: {path}')
    if output in required or output == data_dir / 'sample_submission.csv':
    ...
    with tempfile.TemporaryDirectory(prefix='lgaimers-inference-') as directory:
        ...
        shutil.copy2(source / 'model/model.pkl', stage / 'model/model.pkl')
