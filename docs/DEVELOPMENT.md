...
| Final integration | Blend the calibrated pipeline with Calico JM/JOA and a regular-season CatBoost residual ensemble | Incorporate the additional reference component used in the final local package. |
...
| `model/exp021_inference.py` inside local `last.zip` | The rebuilt EXP-021-style temporal/residual branch informed by mk-isos. |
| `model/current_inference.py` inside local `last.zip` | The calibrated combination of the existing branches. |
| `script.py`, `model/jm0750_inference.py`, and its residual manifest inside local `last.zip` | The actual final two-branch blend and the Calico JM regular-season residual component. |
...
The organizer supplied the baseline and data specification. The project's work includes its feature development, multiclass modeling, experiments, reference-method implementation, calibration, integration, and packaging. The mk-isos method and Calico JM/JOA components are attributed explicitly; their inclusion is not a claim that the project independently originated all of their internals.
