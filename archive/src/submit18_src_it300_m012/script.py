...
NTREE_END = 300   # exp/102: 학습된 8시드 모델의 앞 300개 트리만 사용 (IT 학습곡선 정점, 재학습 없음)
SHIFT_DELTA = 0.012   # exp/102: 2025 중심 프로브 상수 (행 독립). 0.0 이면 무효과
...
def main():
    ...
    preds = np.clip(preds - SHIFT_DELTA, 0.0, 1.0)   # exp/102: 상수 차감 (각 행 독립, 다른 행 미참조)
    if len(preds):
