...
NTREE_END = 300   # 예측에 사용할 트리 수 (학습된 8시드 모델 각 500트리 이하; 500 = 전체 사용)
SCALE_S = 1.06      # 예측 확률 선형 재조정 상수 (고정값, 각 행 독립 적용)
CENTER_C = 0.49    # 재조정 중심 (학습 데이터 2024 성공률 .486 근처의 고정 상수)
SHIFT_DELTA = 0.0066   # 학습 데이터(train.csv) 연도별 성공률 하락 추세(2019 .565 → 2024 .486, 연평균 −0.016)에 근거한
...
def main():
    ...
    v18_tables = np.load(os.path.join(MODEL_DIR, "v18_tables.npz"), allow_pickle=False)
    models = b["cb_models"]
    feats = b["feats"]
    prior = b["prior"]
    ...
    preds = np.clip(CENTER_C + SCALE_S * (preds - CENTER_C) - SHIFT_DELTA, 0.0, 1.0)   # 고정 상수 선형 재조정 — 행 독립 (행 하나만 있어도 결과 동일)
    if len(preds):
