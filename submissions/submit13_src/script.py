...
def main():
    ...
    models = b["cb_models"]          # CS79 8시드
    m65 = b["cb65_models"]           # CB65 8시드 (LB 898.62 검증본)
    w65 = float(b["w65"])
    feats = b["feats"]
    prior = b["prior"]
    print(f"  cs79={len(models)}  cb65={len(m65)}  w65={w65}  "
          f"features={len(feats)}  prior={prior:.6f}")
    ...
    print(f"Inference ({len(models)}+{len(m65)} models)...")
    if len(test) == 0:
        ...
        for i, m in enumerate(models, 1):
            ...
            print(f"  cs79 model {i}/{len(models)} done")
        # CB65 경로: 65피처 프레임 (구종·CS 피처 미사용 — 학습 당시와 동일 입력)
        X65 = build_matrix(ft, b["feats65"])
        acc65 = np.zeros(len(test), dtype=np.float64)
        for i, m in enumerate(m65, 1):
            acc65 += m.predict_proba(X65)[:, 1]
            print(f"  cb65 model {i}/{len(m65)} done")
        preds = (1.0 - w65) * (acc / len(models)) + w65 * (acc65 / len(m65))
