"""Row-local production features for the psychological/latent-pitch residual."""
...
def apply_linear_residual(features,meta_path):
    z=np.load(meta_path,allow_pickle=True);cols=z['columns'].tolist();x=features.reindex(columns=cols).astype(float)
    a=((x-z['mean'])/z['std']).fillna(0).to_numpy();return a@z['coef']*float(z['scale'])
